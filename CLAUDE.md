...
zipをリポジトリに手で展開しない。`binary/index.html` などをルートに置いてしまう事故が起きる。

```bash
python3 _tools/apply-zip.py joholab_v8.zip          # 確認のみ
python3 _tools/apply-zip.py joholab_v8.zip --apply  # 反映する
```

スクリプトがやること:
- 各ファイルの canonical URL と置き先のパスが合っているか検証し、
  食い違ったら反映を中止する（取り違えの防止）
- 現在あるのに新版から消えている目印を警告する（巻き戻しの検出）
- 反映後、zipを `_work/` に片づける

警告が出たら、そのまま `--apply` せず、巻き戻して良いかを確かめる。
**zipは必ず最新のリポジトリ内容から作ってもらう**こと。古い版から作られたzipは、
過去の修正を巻き戻す。
